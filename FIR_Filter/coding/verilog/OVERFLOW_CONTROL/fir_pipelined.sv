// ============================================================================
// fir_pipelined.sv — Pipelined Adder-Tree FIR Filter (symmetric, linear phase)
//                    OVERFLOW CONTROL variant: true 32-bit internal tree
// ============================================================================
// Same datapath as fir_basic.sv but with PIPELINE REGISTERS at every stage
// of the binary adder tree.  7 registered stages, greatly improving Fmax.
//
// Pipeline latency: 8 clock cycles (1 product reg + 7 tree stages)
// Throughput:       1 sample/clock at a much higher Fmax.
//
...
module fir_pipelined #(
    ...
    // =====================================================================
    //  PIPELINED BINARY ADDER TREE (32-bit, two's complement wrap)
    // =====================================================================
    ...
    // Stages 1..7
    reg signed [W_OUT-1:0] tree_s1 [0:(N_PADDED/2)-1];
    reg signed [W_OUT-1:0] tree_s2 [0:(N_PADDED/4)-1];
    reg signed [W_OUT-1:0] tree_s3 [0:(N_PADDED/8)-1];
    reg signed [W_OUT-1:0] tree_s4 [0:(N_PADDED/16)-1];
    reg signed [W_OUT-1:0] tree_s5 [0:(N_PADDED/32)-1];
    reg signed [W_OUT-1:0] tree_s6 [0:(N_PADDED/64)-1];
    reg signed [W_OUT-1:0] tree_s7;
