// ============================================================================
// fir_parallel_L3.sv — L=3 Polyphase Parallel FIR Filter
...
//
// Multiplier count: 288 (192 H_0 + 96 H_1)
...
module fir_parallel_L3 #(
    ...
    // --- Combinational binary adder trees (32-bit, two's complement wrap) ---
    localparam TOTAL_PROD  = SUB_LEN + HALF_H1;  // 96
    localparam TREE_STAGES = $clog2(TOTAL_PROD);  // 7
    localparam N_PAD       = 1 << TREE_STAGES;    // 128

    // y0 tree
    reg signed [W_OUT-1:0] y0_t0 [0:N_PAD-1];
    reg signed [W_OUT-1:0] y0_t1 [0:(N_PAD/2)-1];
    reg signed [W_OUT-1:0] y0_t2 [0:(N_PAD/4)-1];
    reg signed [W_OUT-1:0] y0_t3 [0:(N_PAD/8)-1];
    reg signed [W_OUT-1:0] y0_t4 [0:(N_PAD/16)-1];
    reg signed [W_OUT-1:0] y0_t5 [0:(N_PAD/32)-1];
    reg signed [W_OUT-1:0] y0_t6 [0:(N_PAD/64)-1];

    // y1 tree
    reg signed [W_OUT-1:0] y1_t0 [0:N_PAD-1];
    reg signed [W_OUT-1:0] y1_t1 [0:(N_PAD/2)-1];
    reg signed [W_OUT-1:0] y1_t2 [0:(N_PAD/4)-1];
    reg signed [W_OUT-1:0] y1_t3 [0:(N_PAD/8)-1];
    reg signed [W_OUT-1:0] y1_t4 [0:(N_PAD/16)-1];
    reg signed [W_OUT-1:0] y1_t5 [0:(N_PAD/32)-1];
    reg signed [W_OUT-1:0] y1_t6 [0:(N_PAD/64)-1];

    // y2 tree
    reg signed [W_OUT-1:0] y2_t0 [0:N_PAD-1];
    reg signed [W_OUT-1:0] y2_t1 [0:(N_PAD/2)-1];
    reg signed [W_OUT-1:0] y2_t2 [0:(N_PAD/4)-1];
    reg signed [W_OUT-1:0] y2_t3 [0:(N_PAD/8)-1];
    reg signed [W_OUT-1:0] y2_t4 [0:(N_PAD/16)-1];
    reg signed [W_OUT-1:0] y2_t5 [0:(N_PAD/32)-1];
    reg signed [W_OUT-1:0] y2_t6 [0:(N_PAD/64)-1];

    reg signed [W_OUT-1:0] sum_y0, sum_y1, sum_y2;
