// ============================================================================
// tb_fir_parallel_L3_pipelined.sv — Testbench for fir_parallel_L3_pipelined
// ============================================================================
// Same stimulus as tb_fir_parallel_L3 but with extra margin for the 8-cycle
// pipeline latency introduced by the pipelined adder trees.
// Expected: bit-identical output to fir_parallel_L3, delayed by 8 clocks.
// ============================================================================
`timescale 1ns / 1ps

module tb_fir_parallel_L3_pipelined;
    ...
    initial begin
`ifndef MODELSIM_VCD
        $dumpfile("fir_parallel_L3_pipelined.vcd");
        $dumpvars(0, tb_fir_parallel_L3_pipelined);
`endif
        ...
        //                  all zero for k >= 1
        //
        // Expected interleaved outputs (after 8-cycle pipeline):
        ...
        sample_cnt = 0;
        // Extra +8 margin for pipeline latency
        ...
        sample_cnt = 0;
        for (k = 0; k < (NUM_TAPS/3) + 30; k = k + 1) begin
            @(posedge clk);
            if (dout_valid) begin
