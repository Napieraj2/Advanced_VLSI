...
The standardized `.sta.rpt` files make the dominant setup-path behavior clear when read together with the fitted architectures. For the three non-pipelined DSP-based designs (`fir_basic`, `fir_parallel_L2`, and `fir_parallel_L3`), the limiting path remains the arithmetic chain from pre-add into multiplication and then through a wide combinational reduction tree. That is why all three cluster in the same timing range despite different throughput: about 38.7 MHz for the direct form, 37.1 MHz for the L=2 version, and 36.4 MHz for the L=3 version.

Among those non-pipelined variants, the L=3 parallel design is the clearest example of routing pressure amplifying logic depth. Its average interconnect congestion reaches 12.0% and its peak vertical congestion reaches 62.2% in *Table 12*, both of which are substantially higher than the other DSP-based designs. So its worst setup path is not just a long adder tree in isolation; it is a long adder tree embedded in the heaviest routing environment of the design set.

Pipelining changes that picture immediately. In `fir_pipelined`, `fir_parallel_L3_pipelined`, and `fir_mcm_pipelined`, the tree is cut into registered stages, so the critical path collapses from a full reduction tree to a local stage consisting of product generation plus one registered add level. That is why all three pipelined designs converge tightly around 58.3 to 58.7 MHz even though their area and power costs differ substantially.

The MCM direct-form design shows the other important critical-path case. Its worst setup path is longer than the DSP direct form because the coefficient multiplication is implemented as CSD shift-add logic before the sum ever reaches the adder tree. In other words, the direct-form MCM path pays both the cost of a deeper multiplier replacement network and the cost of the non-pipelined reduction tree, which is why it drops to 33.2 MHz, the lowest setup-limited Fmax in the comparison.
